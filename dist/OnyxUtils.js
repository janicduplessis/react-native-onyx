...
function tryGetCachedValue(key, mapping = {}) {
    if (isComputedKey(key)) {
        ...
        let val = OnyxCache_1.default.getValue(key.cacheKey);
        if (val !== undefined) {
            return val;
        }
        // Check if we can compute the value if all dependencies are in cache.
        const dependencies = underscore_1.default.mapObject(key.dependencies || {}, (dependencyKey) => tryGetCachedValue(dependencyKey, 
        // TODO: We could support full mapping here.
        { key: dependencyKey }));
        if (underscore_1.default.all(dependencies, (dependency) => dependency !== undefined)) {
            val = key.compute(dependencies);
            OnyxCache_1.default.set(key.cacheKey, val);
            return val;
        }
        return undefined;
    }
...
function clearComputedCacheForKey(key) {
    ...
    dependentKeys.forEach((dependentKey) => {
        OnyxCache_1.default.drop(dependentKey);
