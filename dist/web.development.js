...
function tryGetCachedValue(key) {let mapping = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  if (isComputedKey(key)) {
    ...
    let val = _OnyxCache__WEBPACK_IMPORTED_MODULE_4__["default"].getValue(key.cacheKey);
    if (val !== undefined) {
      return val;
    }

    // Check if we can compute the value if all dependencies are in cache.
    const dependencies = underscore__WEBPACK_IMPORTED_MODULE_1___default().mapObject(key.dependencies || {}, (dependencyKey) =>
    tryGetCachedValue(
    dependencyKey,
    ...
    if (underscore__WEBPACK_IMPORTED_MODULE_1___default().all(dependencies, (dependency) => dependency !== undefined)) {
      val = key.compute(dependencies);
      _OnyxCache__WEBPACK_IMPORTED_MODULE_4__["default"].set(key.cacheKey, val);
      return val;
    }
...
function clearComputedCacheForKey(key) {
  ...
  dependentKeys.forEach((dependentKey) => {
    _OnyxCache__WEBPACK_IMPORTED_MODULE_4__["default"].drop(dependentKey);
...
function computeAndSendData(mapping, dependencies) {
  let val = _OnyxCache__WEBPACK_IMPORTED_MODULE_4__["default"].getValue(mapping.key.cacheKey);
  if (val === undefined) {
    val = mapping.key.compute(dependencies);
    _OnyxCache__WEBPACK_IMPORTED_MODULE_4__["default"].set(mapping.key.cacheKey, val);
  }
...
function connect(mapping) {
  ...
  if (isComputedKey(mapping.key)) {
    ...
    then(() => {
      ...
      const dependenciesCount = underscore__WEBPACK_IMPORTED_MODULE_1___default().size(mappingDependencies);
      if (dependenciesCount === 0) {
        ...
        const dependencies = {};
        underscore__WEBPACK_IMPORTED_MODULE_1___default().each(mappingDependencies, (dependency, mappingKey) => {
          ...
          const dependencyConnection = connect({
            ...
            callback: (value) => {
              ...
              if (underscore__WEBPACK_IMPORTED_MODULE_1___default().size(dependencies) === dependenciesCount) {
    ...
    class withOnyx extends (react__WEBPACK_IMPORTED_MODULE_1___default().Component) {
      ...
      componentDidUpdate(prevProps, prevState) {
        ...
        underscore__WEBPACK_IMPORTED_MODULE_2___default().each(mapOnyxToState, (mapping, propName) => {
          ...
          const previousKey = _Onyx__WEBPACK_IMPORTED_MODULE_4__["default"].getCacheKey(isFirstTimeUpdatingAfterLoading ? mapping.previousKey : _Str__WEBPACK_IMPORTED_MODULE_3__.result(mapping.key, { ...prevProps, ...prevOnyxDataFromState }));
          const newKey = _Onyx__WEBPACK_IMPORTED_MODULE_4__["default"].getCacheKey(_Str__WEBPACK_IMPORTED_MODULE_3__.result(mapping.key, { ...this.props, ...onyxDataFromState }));
